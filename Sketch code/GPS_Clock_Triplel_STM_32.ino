...
#define TICK_PITCH            2000                 // frequency (Hz) of hour toneTo provide your LOTW confirmation is quite easy to produce.                                                         
...
// added below by FAH 09/01/2024
void showGridSquareD(int x, int y) {
  const int f = 4;                                 // text font
  char gs[12];                                     // buffer for new grid square
  if (!gps.location.isValid()) return;             // leave if no fix
  float lat = gps.location.lat();                  // get latitude
  float lon = gps.location.lng();                  // and longitude
  getGridSquare(gs, lat, lon, GRID_SQUARE_SIZE);   // compute current grid square
  //  Display grid square all the time  ******************** added July 4, 2022 KenK
...
void showCallsignD(int x, int y ) {
  const int f = 4;
  tft.setTextColor(LABEL_FGCOLOR, LABEL_BGCOLOR);
  tft.drawString(callsign, x, y, f);
}
// added above by FAH 09/01/2024
...
void updateDualScreen() {
  ...
  showCallsignD(callsignPos, 2);                    // added 9/2/2024 FAH
  showGridSquareD(120, 128);                        // added 9/2/2024 FAH
}
